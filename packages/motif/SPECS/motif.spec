...
Patch100: motif.sgifixbzero.patch
...
install -d %{buildroot}%{_prefix}/etc/X11/xinit/xinitrc.d
install -m 755 %{SOURCE1} %{buildroot}%{_prefix}/etc/X11/xinit/xinitrc.d/xmbind.sh

rm -f %{buildroot}%{_libdir}/*.la

# Move some things that get dropped in "system" places
mkdir -p $RPM_BUILD_ROOT%{_prefix}/etc/X11/mwm
mv $RPM_BUILD_ROOT/etc/X11/mwm/system.mwmrc $RPM_BUILD_ROOT%{_prefix}/etc/X11/mwm/
...
%{_prefix}/etc/X11/xinit/xinitrc.d/xmbind.sh
%dir %{_prefix}/etc/X11/mwm
%config(noreplace) %{_prefix}/etc/X11/mwm/system.mwmrc
%{_bindir}/mwm
%{_bindir}/xmbind
...
* Sun Apr 19 2020 Daniel Hams <daniel.hams@gmail.com> - 2.3.4-19
- Import into wip
