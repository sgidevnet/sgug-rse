...
do
  %{_bindir}/iconv -f ISO-8859-1 -t UTF-8 "$fhtml" > "$fhtml".tmp
  %{_bindir}/mv -f "$fhtml".tmp "$fhtml"
  %{_bindir}/sed -i 's/charset=ISO-8859-1/charset=UTF-8/' "$fhtml"
done

for f in $(find -name "*'*")
do
  %{_bindir}/mv -v "$f" $(echo "$f" | tr -d "'")
done
