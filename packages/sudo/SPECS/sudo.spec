...
Patch101: sudo.sgifixes01.patch
Patch102: sudo.sgifixes02.patch
...
export LT_SYS_LIBRARY_PATH="%{_libdir}:/usr/lib32:/lib32"
...
%configure \
        ...
        --without-ldap \
        --without-selinux \
        --with-passprompt="[sudo] assword for %p: " \
        --with-all-insults \
        --with-sssd \
...
install -p -d -m 700 $RPM_BUILD_ROOT%{_prefix}/var/db/sudo
install -p -d -m 700 $RPM_BUILD_ROOT%{_prefix}/var/db/sudo/lectured
install -p -d -m 750 $RPM_BUILD_ROOT%{_prefix}/etc/sudoers.d
install -p -c -m 0440 %{SOURCE1} $RPM_BUILD_ROOT%{_prefix}/etc/sudoers

# sudoers rewrites
# (1) Rewrite the secure_path to use our prefix
perl -pi -e "s|/usr/local/sbin:/usr/local/bin|%{_sbindir}:%{_bindir}|g" $RPM_BUILD_ROOT%{_prefix}/etc/sudoers

# (2) Rewrite the linuxy "wheel" to be the irix "sys" group
perl -pi -e "s|wheel|sys|g" $RPM_BUILD_ROOT%{_prefix}/etc/sudoers

# (3) Rewrite the includedir reference
perl -pi -e "s|/etc/sudoers.d|%{_prefix}/etc/sudoers.d|g" $RPM_BUILD_ROOT%{_prefix}/etc/sudoers
...
%attr(0440,root,root) %config(noreplace) %{_prefix}/etc/sudoers
%attr(0750,root,root) %dir %{_prefix}/etc/sudoers.d/
#%config(noreplace) %{_prefix}/etc/pam.d/sudo
#%config(noreplace) %{_prefix}/etc/pam.d/sudo-i
#%attr(0644,root,root) %{_tmpfilesdir}/sudo.conf
#%attr(0644,root,root) %{_prefix}/etc/dnf/protected.d/sudo.conf
%dir %{_prefix}/var/db/sudo
%dir %{_prefix}/var/db/sudo/lectured
...
%{_libexecdir}/sudo/libsudo_util.so
%{_mandir}/man4/sudoers.4*
#{_mandir}/man4/sudoers.ldap.4*
%{_mandir}/man4/sudo.conf.4*
%{_mandir}/man1m/sudo.1m*
%{_mandir}/man1m/sudoedit.1m*
%{_mandir}/man1m/sudoreplay.1m*
%{_mandir}/man1m/visudo.1m*
%{_mandir}/man1/cvtsudoers.1.gz
%{_mandir}/man4/sudoers_timestamp.4.gz
%dir %{_pkgdocdir}/
%{_pkgdocdir}/*
...
%post
%{_bindir}/chmod 0440 %{_prefix}/etc/sudoers || :
