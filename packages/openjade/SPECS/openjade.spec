...
Patch1000: openjade.sgifixes.patch
...
# this package uses a very old and broken (for irix) libtool
# so let's force use of ours
export SGUG_LIBTOOL="%{_bindir}/libtool --tag=CXX"
...
# Need to set this here too, since install is in a separate shell fragment
export SGUG_LIBTOOL="%{_bindir}/libtool --tag=CXX"
make %{_smp_mflags} install install-man DESTDIR=$RPM_BUILD_ROOT
...
rm -f $RPM_BUILD_ROOT%{_libdir}/*.so $RPM_BUILD_ROOT%{_libdir}/*.la $RPM_BUILD_ROOT%{_libdir}/*.a
