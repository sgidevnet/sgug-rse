...
%build
# Rewrite some stuff we need rewritten before running configure
perl -pi -e "s|/etc/sgml|%{_sysconfdir}/sgml|g" doc/HTML/Makefile.am
perl -pi -e "s|/etc/sgml|%{_sysconfdir}/sgml|g" doc/HTML/Makefile.in
perl -pi -e "s|/etc/sgml|%{_sysconfdir}/sgml|g" doc/man/Makefile.am
perl -pi -e "s|/etc/sgml|%{_sysconfdir}/sgml|g" doc/man/Makefile.in

perl -pi -e "s|/etc/sgml|%{_sysconfdir}/sgml|g" bin/jw.in
...
perl -pi -e "s|/usr/bin/perl|%{_bindir}/perl|g" $RPM_BUILD_ROOT%{_bindir}/sgmldiff

# Remove files for the pdf package (which we can't install yet)
rm $RPM_BUILD_ROOT%{_bindir}/docbook2pdf
rm $RPM_BUILD_ROOT%{_bindir}/docbook2dvi
rm $RPM_BUILD_ROOT%{_bindir}/docbook2ps
rm $RPM_BUILD_ROOT%{_bindir}/db2dvi
rm $RPM_BUILD_ROOT%{_bindir}/db2pdf
rm $RPM_BUILD_ROOT%{_bindir}/db2ps
rm $RPM_BUILD_ROOT%{_mandir}/*/db2pdf.*
rm $RPM_BUILD_ROOT%{_mandir}/*/docbook2pdf.*
rm $RPM_BUILD_ROOT%{_mandir}/*/docbook2dvi.*
rm $RPM_BUILD_ROOT%{_mandir}/*/docbook2ps.*
