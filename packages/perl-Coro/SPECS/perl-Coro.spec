...
Patch100:       perl-Coro.sgifixes.patch
...
documentation). They are similar to kernel threads but dont (in general) run
...
# use ucontext backend on non-x86 (setjmp didnt work on s390(x))
...
# Disable for IRIX, too
#%%ifarch %{arm}
RPM_OPT_FLAGS="${RPM_OPT_FLAGS} -Wp,-U_FORTIFY_SOURCE -Wp,-D_FORTIFY_SOURCE=0"
#%%endif

# Interactive config, use specific values to get pthread functionality
printf "y\nn\np\n16384\n4\nn\nn\ny\nn\n" >automatedanswers.txt
perl Makefile.PL INSTALLDIRS=perl NO_PACKLIST=1 NO_PERLLOCAL=1 \
    OPTIMIZE="$RPM_OPT_FLAGS" <automatedanswers.txt
%{make_build}
