...
Patch100: ghostscript.sgifixes.patch
...
sed -r -i 's|#!/bin/sh|#!/usr/sgug/bin/sh|' %{_builddir}/ghostscript-9.27/lib/*
sed -r -i 's|#!/bin/sh|#!/usr/sgug/bin/sh|' %{_builddir}/ghostscript-9.27/toolbin/tmake.tcl
sed -r -i 's|#!/bin/sh|#!/usr/sgug/bin/sh|' %{_builddir}/ghostscript-9.27/toolbin/gsmake
sed -r -i 's|#!/bin/sh|#!/usr/sgug/bin/sh|' %{_builddir}/ghostscript-9.27/toolbin/errlist.tcl
sed -r -i 's|#!/bin/sh|#!/usr/sgug/bin/sh|' %{_builddir}/ghostscript-9.27/toolbin/gitsetup.sh
sed -r -i 's|#!/bin/sh|#!/usr/sgug/bin/sh|' %{_builddir}/ghostscript-9.27/toolbin/tests/run_nightly
sed -r -i 's|#!/bin/sh|#!/usr/sgug/bin/sh|' %{_builddir}/ghostscript-9.27/base/*
sed -r -i 's|#!/bin/sh|#!/usr/sgug/bin/sh|' %{_builddir}/ghostscript-9.27/autogen.sh
sed -r -i 's|/bin/sh|/usr/sgug/bin/sh|' %{_builddir}/ghostscript-9.27/base/unixhead.mak
sed -r -i 's|/bin/sh|/usr/sgug/bin/sh|' %{_builddir}/ghostscript-9.27/base/all-arch.mak
sed -r -i 's|/bin/sh|/usr/sgug/bin/sh|' %{_builddir}/ghostscript-9.27/configure
...
* Sat Jun 27 2020 Peter Robinson <pbrobinson@fedoraproject.org> - 9.27-8
