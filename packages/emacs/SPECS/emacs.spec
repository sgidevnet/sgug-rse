...
#rm build-{motif,lucid,nox}/src/emacs-%{version}.*
...
%%_emacs_bytecompile %{_bindir}/emacs -batch --no-init-file --no-site-file --eval '(progn (setq load-path (cons "." load-path)))' -f batch-byte-compile
EOF
...
# Remove some chaff
rm %{buildroot}%{_bindir}/b2m
rm %{buildroot}%{_bindir}/grep-changelog
rm %{buildroot}%{_bindir}/rcs-checkin

rm %{buildroot}%{_prefix}/var/games/emacs/snake-scores
rm %{buildroot}%{_prefix}/var/games/emacs/tetris-scores
