...
for all in CHANGES README; do
    iconv -f ISO8859-1 -t UTF-8 < $all > $all.new
    touch -r $all $all.new
    mv -f $all.new $all
done

# Rewrite some references to /bin/sh
perl -pi -e "s|/bin/sh|%{_bindir}/sh|g" Makefile.in
perl -pi -e "s|/bin/sh|%{_bindir}/sh|g" Source/Makefile.in
perl -pi -e "s|/bin/sh|%{_bindir}/sh|g" Lib/perl5/Makefile.in
perl -pi -e "s|/bin/sh|%{_bindir}/sh|g" Lib/python/Makefile.in
