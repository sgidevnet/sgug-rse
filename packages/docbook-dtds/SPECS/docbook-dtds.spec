...
%{_bindir}/sed -e's,\(NAMELEN\s\+\)44\(\s\*\)\?,\1256,' -i.namelen */docbook.dcl

# fix of \r\n issue from rpmlint
%{_bindir}/sed -i 's/\r//' */*.txt
...
# Symlinks
mkdir -p $RPM_BUILD_ROOT%{_prefix}/etc/sgml
for fmt in sgml xml; do
  ln -s $fmt-docbook-4.5.cat \
     $RPM_BUILD_ROOT%{_prefix}/etc/sgml/$fmt-docbook.cat
done
...
do
  ...
  fmt=${dir#*-} ver=${dir%%-*}
  case $fmt in
    sgml)   DESTDIR=$RPM_BUILD_ROOT%{_prefix}/share/sgml/docbook/$fmt-dtd-$ver ;;
    xml)    DESTDIR=$RPM_BUILD_ROOT%{_prefix}/share/sgml/docbook/$fmt-dtd-$ver ;;
    rng)    DESTDIR=$RPM_BUILD_ROOT%{_prefix}/share/sgml/docbook/$fmt-$ver ;;
    xsd)    DESTDIR=$RPM_BUILD_ROOT%{_prefix}/share/sgml/docbook/$fmt-$ver ;;
  esac
  case $fmt in
...
do
  cd $dir
  fmt=${dir#*-} ver=${dir%%-*}
  DESTDIR=$RPM_BUILD_ROOT%{_prefix}/share/sgml/docbook/$fmt-dtd-$ver
  install *.dtd *.mod $DESTDIR
  install docbook.cat $DESTDIR/catalog
  cd ..
  # File for %%ghost
  touch $RPM_BUILD_ROOT%{_prefix}/etc/sgml/$fmt-docbook-$ver.cat
done
...
%{_pkgdocdir}
%{_prefix}/share/sgml/docbook/*ml-dtd-*
%{_prefix}/share/sgml/docbook/rng-*
%{_prefix}/share/sgml/docbook/xsd-*
%config(noreplace) %{_prefix}/etc/sgml/*ml-docbook.cat
%ghost %config(noreplace) %{_prefix}/etc/sgml/*ml-docbook-*.cat

%post
catcmd='%{_bindir}/xmlcatalog --noout'
xmlcatalog=%{_prefix}/share/sgml/docbook/xmlcatalog
...
for STYLESHEETS in %{_prefix}/share/sgml/docbook/dsssl-stylesheets-*; do : ; done
case $STYLESHEETS in
  *-"*") STYLESHEETS= ;;
esac
eval set %{catalog_list}
...
do
  fmt=${dir#*-} ver=${dir%%-*}
  sgmldir=%{_prefix}/share/sgml/docbook/$fmt-dtd-$ver
  ## SGML catalog
  # Update the centralized catalog corresponding to this version of the DTD
  for cat_dir in %{_prefix}/share/sgml/sgml-iso-entities-8879.1986 $sgmldir $STYLESHEETS; do
    $catcmd --sgml --add %{_prefix}/etc/sgml/$fmt-docbook-$ver.cat $cat_dir/catalog
  done
  ## XML catalog
...
%{_bindir}/sed -ni '
  ...
  ' %{_prefix}/etc/sgml/catalog

# Finally, make sure everything in /etc/sgml is readable!
%{_bindir}/chmod a+r %{_prefix}/etc/sgml/*
...
ent/iso-cyr2.ent
  ...
  do
    ...
    ## SGML catalog
    # Update the centralized catalog corresponding to this version of the DTD
    $catcmd --sgml --del %{_prefix}/etc/sgml/catalog %{_prefix}/etc/sgml/$fmt-docbook-$ver.cat
    rm -f %{_prefix}/etc/sgml/$fmt-docbook-$ver.cat
    ## XML catalog
    if [ $fmt = xml -a -w $xmlcatalog ]; then
...
do
  fmt=${dir#*-} ver=${dir%%-*}
  %{_bindir}/xmlcatalog --sgml --noout --add %{_prefix}/etc/sgml/$fmt-docbook-$ver.cat \
    %{_prefix}/share/sgml/openjade-%{openjadever}/catalog
done
...
do
  fmt=${dir#*-} ver=${dir%%-*}
  %{_bindir}/xmlcatalog --sgml --noout --del %{_prefix}/etc/sgml/$fmt-docbook-$ver.cat \
    %{_prefix}/share/sgml/openjade-%{openjadever}/catalog
done
